...
masses_ref_2016 = [80, 90, 100, 110, 120, 130, 140, 160, 180, 200, 250, 300, 350, 400, 450, 500, 600, 700, 800, 900, 1000, 1200, 1400, 1500, 1600, 1800, 2000, 2300, 2600, 2900, 3200]

if args.year == '2016': masses_ref = masses_ref_2016
else: masses_ref = masses
...
for m in masses_ref:
    ...
    wsptools.SafeWrapHist(w, ['h_pt'],
        GetNormed('input/mssm_pt_weight_inputs.root:ref_m%(m)s%(extra)s' % vars()), name='h_%(m)s_ref' % vars())
    ...
    for P in ['h', 'A', 'H']:
      for u in ['','_scale_up','_scale_down','_hdamp_up','_hdamp_down']:
        ...
        # define binned weights here for mass points that exist for both ref and powheg 
        w.factory('expr::%(P)s_%(m)s_t_ratio%(u)s("@0/@1", %(P)s_%(m)s_t%(u)s, h_%(m)s_ref)' % vars())
        w.factory('expr::%(P)s_%(m)s_b_ratio%(u)s("@0/@1", %(P)s_%(m)s_b%(u)s, h_%(m)s_ref)' % vars())
        w.factory('expr::%(P)s_%(m)s_i_ratio%(u)s("@0/@1", %(P)s_%(m)s_i%(u)s, h_%(m)s_ref)' % vars())

# now use linear interpolation between mass points
masses_common = list(set(masses_ref+masses_ph).intersection(masses_ref).intersection(masses_ph))
masses_common.sort()
